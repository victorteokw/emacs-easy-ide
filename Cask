(source gnu)
(source melpa)
(source melpa-stable)
(source org)

(depends-on "ac-c-headers")
(depends-on "ac-html")
(depends-on "ac-inf-ruby")
(depends-on "ace-jump-mode")
(depends-on "aggressive-indent")
(depends-on "anzu")
(depends-on "async")
(depends-on "auto-complete")
(depends-on "auto-complete-c-headers")
(depends-on "auto-complete-clang-async")
(depends-on "avy")
(depends-on "browse-kill-ring")
(depends-on "buffer-move")
(depends-on "bundler")
(depends-on "calfw")
(depends-on "cask")
(depends-on "coffee-mode")
(depends-on "company")
(depends-on "company-irony")
(depends-on "company-irony-c-headers")
(depends-on "dash")
(depends-on "dash-at-point")
(depends-on "diff-hl")
(depends-on "discover")
(depends-on "e2wm")
(depends-on "electric-operator")
(depends-on "elisp-slime-nav")
(depends-on "elnode")
(depends-on "emmet-mode")
(depends-on "evil")
(depends-on "exec-path-from-shell")
(depends-on "expand-line")
(depends-on "expand-region")
(depends-on "f")
(depends-on "feature-mode")
(depends-on "flycheck")
(depends-on "ggtags")
(depends-on "gh")
(depends-on "git-commit")
(depends-on "git-messenger")
(depends-on "git-timemachine")
(depends-on "git-wip-timemachine")
(depends-on "gitconfig-mode")
(depends-on "github-browse-file")
(depends-on "gitignore-mode")
(depends-on "google-maps")
(depends-on "goto-gem")
(depends-on "haml-mode")
(depends-on "health" :git "https://github.com/cheunghy/emacs-health.git")
(depends-on "helm")
(depends-on "helm-core")
(depends-on "helm-dash")
(depends-on "helm-projectile")
(depends-on "helm-swoop")
(depends-on "highlight-escape-sequences")
(depends-on "highlight-symbol")
(depends-on "hl-sexp")
(depends-on "hlinum")
(depends-on "ido-completing-read+")
(depends-on "ido-ubiquitous")
(depends-on "iedit")
(depends-on "indent-guide")
(depends-on "inf-php")
(depends-on "inf-ruby")
(depends-on "irony")
(depends-on "irony-eldoc")
(depends-on "js2-mode")
(depends-on "json-mode")
(depends-on "json-reformat")
(depends-on "jst")
(depends-on "less-css-mode")
(depends-on "let-alist")
(depends-on "linum-off")
(depends-on "litable")
(depends-on "macrostep")
(depends-on "magit")
(depends-on "magit-popup")
(depends-on "markdown-mode")
(depends-on "minitest")
(depends-on "move-dup")
(depends-on "multiple-cursors")
(depends-on "names")
(depends-on "nodejs-repl")
(depends-on "org")
(depends-on "origami")
(depends-on "package-build")
(depends-on "pallet")
(depends-on "paredit")
(depends-on "php-mode")
(depends-on "popup")
(depends-on "projectile")
(depends-on "projectile-rails")
(depends-on "rails-new")
(depends-on "rainbow-delimiters")
(depends-on "rainbow-mode")
(depends-on "rake")
(depends-on "readline-complete")
(depends-on "restart-emacs")
(depends-on "restclient")
(depends-on "rich-minority")
(depends-on "rspec-mode")
(depends-on "ruby-compilation")
(depends-on "s")
(depends-on "sass-mode")
(depends-on "scratches" :git "https://github.com/cheunghy/scratches.git")
(depends-on "scss-mode")
(depends-on "slim-mode")
(depends-on "smart-indent-mode" :git "https://github.com/cheunghy/smart-indent-mode.git")
(depends-on "smart-mark")
(depends-on "smartscan")
(depends-on "smex")
(depends-on "sunshine")
(depends-on "swift-mode")
(depends-on "typescript-mode")
(depends-on "uju-theme" :git "https://github.com/cheunghy/emacs-uju-theme.git")
(depends-on "undo-tree")
(depends-on "web")
(depends-on "web-mode")
(depends-on "which-key")
(depends-on "whole-line-or-region")
(depends-on "with-editor")
(depends-on "yaml-mode")
(depends-on "yard-mode")
(depends-on "yari")
(depends-on "yasnippet")
